...
The current timeline has nine scenes and runs 87 seconds. Its content order is `cover`, Background (`background`), Question (`actions`), Insight (`efficiency`), Mechanism (`method`), Evidence (`simulation`, `realworld`), Vision (`summary`), and `back`.
