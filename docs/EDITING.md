...
The current nine-scene order is `cover` → `background` → `actions` → `efficiency` → `method` → `simulation` → `realworld` → `summary` → `back`. Its scientific spine is Background → Problem → Method → Evaluation → Takeaway; preserve that order when revising copy or timing.
...
Adding a scene is exceptional: first check whether its point can replace secondary copy in the existing Background → Problem → Method → Evaluation → Takeaway structure while keeping the cut in the 82–87 second working range.
